...
export default class DinoGame extends GameRunner {
  ...
  constructor(width, height) {
    ...
    this.defaultSettings = {
      ...
      cactiSpawnRate: 60, // fpa
      cloudSpawnRate: 250, // fpa
      cloudSpeed: 3, // ppf
      dinoGravity: 0.5, // ppf
  ...
  async preload() {
    ...
    const [spriteImage] = await Promise.all([
      ...
      loadFont('./assets/PressStart2P-Regular.ttf', 'pressStart2p', 'regular'),
    ])
  ...
  saveScreenshot(callback) {
    ...
    link.download = 'dino-game-score.png'
  ...
  endGame() {
    ...
    const padding = 20
    const smallButtonWidth = this.width / 5
    const smallButtonHeight = smallButtonWidth / 4 - padding
    const gameoverButtonWidth = smallButtonWidth + smallButtonWidth - padding
    const gameoverButtonHeight = smallButtonHeight + smallButtonHeight - padding
    const fontSize = 12
    
    // Sprite - Centered ('name or text', x position, y position)
    const iconSpriteX = this.width / 2 - iconSprite.w / 4
    const iconSpriteY = this.height / 2 - iconSprite.h / 4 + padding
    this.paintSprite(
      'replayIcon',
      iconSpriteX,
      iconSpriteY
    ...
    // GAME OVER BUTTON
    // const gameOverButtonX = this.width / 2 - gameoverButtonWidth / 2
    // const gameOverButtonY = this.height / 2 - iconSprite.h / 2 - gameoverButtonHeight
    const gameOverButtonX = this.width / 2 - iconSprite.w / 4
    const gameOverButtonY = this.height / 2 - iconSprite.h / 4 - padding
    this.canvasCtx.fillStyle = '#ff7b00'
    this.canvasCtx.fillRect(gameOverButtonX, gameOverButtonY, gameoverButtonWidth, gameoverButtonHeight)
    // GAME OVER TEXT
    this.paintText(
      'G A M E  O V E R', this.width / 2, this.height / 2 - iconSprite.h / 2 - gameoverButtonHeight, {
        font: 'pressStart2p',
        size: '12px',
        align: 'center',
        baseline: 'middle',
    ...
    // 'PLAY AGAIN' button
    // const playAgainButtonX = this.width / 2 - smallButtonWidth / 2
    const playAgainButtonX = this.width / 2 - iconSprite.w / 4
    // const playAgainButtonY = this.height / 2 + iconSprite.h / 2
    const playAgainButtonY = this.height / 2 - iconSprite.h / 4 + padding
    this.canvasCtx.fillStyle = '#ff7b00'
    this.canvasCtx.fillRect(playAgainButtonX, playAgainButtonY, smallButtonWidth, smallButtonHeight)
    // Paint PLAY AGAIN text
    this.paintText('PLAY AGAIN', this.width / 2, this.height / 2 + iconSprite.h / 2, {
      font: 'pressStart2p',
      size: '10px',
      align: 'center',
      baseline: 'middle',
    ...
    // 'SAVE SCORE' button
    // const saveScoreButtonX = this.width / 2 - iconSprite.w / 2 - smallButtonWidth
    const saveScoreButtonX = this.width / 2 - iconSprite.w / 4
    // const saveScoreButtonY = this.height / 2 - smallButtonHeight / 2
    const saveScoreButtonY = this.height / 2 - iconSprite.h / 4 + padding + padding
    this.canvasCtx.fillStyle = '#ff7b00'
    this.canvasCtx.fillRect(saveScoreButtonX, saveScoreButtonY, smallButtonWidth, smallButtonHeight)
    // 'SAVE SCORE' TEXT
    this.paintText('SAVE SCORE', this.width / 2 - iconSprite.w / 2 - smallButtonWidth, this.height / 2 - smallButtonHeight / 2, {
      font: 'pressStart2p',
      size: '10px',
      align: 'center',
      baseline: 'middle',
    ...
    // Draw EXIT GAME 'go back' button
    // const goBackButtonX = this.width / 2 + smallButtonWidth / 2
    const goBackButtonX = this.width / 2 - iconSprite.w / 4
    const goBackButtonY = this.height / 2 - smallButtonHeight / 2 + padding + padding
    this.canvasCtx.fillStyle = '#ff7b00'
    this.canvasCtx.fillRect(goBackButtonX, goBackButtonY, smallButtonWidth, smallButtonHeight)
    this.paintText('EXIT GAME', this.width / 2, this.height / 2 - smallButtonHeight / 2, {
      font: 'pressStart2p',
      size: '10px',
      align: 'center',
      baseline: 'middle',
    ...
    // Add a click event listener on the canvas to handle the 'save score' button click
  ...
  handleCanvasClick(event) {
    ...
    // saveButtonWidth = smallButtonWidth
    // saveButtonHeight = smallButtonHeight
    
    // const saveButtonX = this.width / 2 + iconSprite.w - paddingOne - paddingOne
    // const saveButtonY = this.height - iconSprite.h - paddingOne
    // const playButtonX = this.width / 3 - iconSprite.w / 5
    
    // Main consts
    const iconSprite = sprites.replayIcon
    const padding = 20
    const smallButtonWidth = this.width / 5
    const smallButtonHeight = smallButtonWidth / 6
    const gameoverButtonWidth = smallButtonWidth + smallButtonWidth
    const gameoverButtonHeight = smallButtonHeight + smallButtonHeight
    // Font and Canvas consts
    const fontSize = 12
    const rect = this.canvas.getBoundingClientRect()
    const x = event.clientX - rect.left
    const y = event.clientY - rect.top
    // Declare buttons with their correct X and Y values
    const gameOverButtonX = this.width / 2 - gameoverButtonWidth / 2
    const gameOverButtonY = this.height / 2 - iconSprite.h / 2 - gameoverButtonHeight
    const playAgainButtonX = this.width / 2 - smallButtonWidth / 2
    const playAgainButtonY = this.height / 2 + iconSprite.h / 2
    ...
    const saveScoreButtonY = this.height / 2 - smallButtonHeight / 2
    const goBackButtonX = this.width / 2 + smallButtonWidth / 2
    const goBackButtonY = this.height / 2 - smallButtonHeight / 2
    
    // Check if the 'go back' button is clicked
    if (
      x >= goBackButtonX &&
      x <= goBackButtonX + smallButtonWidth &&
      y >= goBackButtonY &&
      y <= goBackButtonY + smallButtonHeight
    ) {
      window.location.href = 'https://extc.me/play'
    }
    
    if (
      x >= saveScoreButtonX &&
      x <= saveScoreButtonX + smallButtonWidth &&
      y >= saveScoreButtonY &&
      y <= saveScoreButtonY + smallButtonHeight
    ) {
      this.saveScreenshot(() => {
        console.log('Score image saved')
      })
    }
    
    if (
      x >= playAgainButtonX &&
      x <= playAgainButtonX + smallButtonWidth &&
      y >= playAgainButtonY - smallButtonHeight / 2 &&
      y <= playAgainButtonY + smallButtonHeight /2
    ) {
      this.resetGame()
    }
  ...
  updateScore() {
    ...
    if (this.frameCount % state.settings.scoreIncreaseRate === 0) {
      ...
      if (state.level !== oldLevel) {
        ...
        state.milestoneText.value = `you've reached ${state.level * 100} points`
        state.milestoneText.frames = state.milestoneText.duration
      }
    }
  }
  ...
  drawScore() {
    ...
    if (shouldDraw) {
      ...
      const paddingTop = 12  // Adjust this value as needed
      this.paintText((drawValue + '').padStart(5, '0'), this.width, paddingTop, {
        color: '#000000',
        font: 'pressStart2p',
        size: `${fontSize}px`,
        align: 'right',
  ...
  displayMilestoneText() {
    const { milestoneText } = this.state
    if (milestoneText.frames > 0) {
      this.paintText(milestoneText.value, this.width / 2, this.height / 2, {
        color: '#000000',
        font: 'pressStart2p',
        size: '12px',
        align: 'center',
 ...
 paintInstances(instances) {
  ...
  paintText(text, x, y, opts) {
    const { font = 'pressStart2p', size = '12px' } = opts
